...
def auxiliary_model_analysis(
    ...
    best_gmm_clf = train_gmm(X_train, components_range=components_range, verbose=verbose)
    if verbose:
        print("---Likelihoods---")
    ...
    if kde:
        if verbose:
            print("=====" * 5 + " Training KDE Model " + "=====" * 5)
        kde_model = train_kde(
            X_train, bandwidth_range=[0.5, 1.0, 1.5, 2.0, 2.5, 3.0], verbose=verbose
        )
    ...
    gmm_clf = Pipeline(
        [
            ...
            (
                "GMM",
                GaussianMixture(
                    init_params="kmeans", covariance_type="full", max_iter=100000
                ),
            ),
        ]
    )
...
def train_kde(X_train, bandwidth_range=[1.0, 1.5, 2.0], verbose=False, pca=False):
...
def ood_metrics(
    ...
    if find_fpr:
        ...
        if np.isclose(tpr, 0.95, rtol=-1e-2, atol=3e-2).any():
            ...
            tpr95_idx = np.where(np.isclose(tpr, 0.95, rtol=2e-1, atol=3e-2))[0][0]
